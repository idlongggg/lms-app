...
export function getLessonsByTopic(topicId: string): Lesson[] {
  return lessons.filter(
    (l) => l.topicId === topicId && l.status === "PUBLISHED",
  );
}
...
export function getUserProgressBySubject(
  userId: string,
  subjectId: string,
): LessonProgress[] {
  ...
  return lessonProgress.filter(
    (p) => p.userId === userId && lessonIds.includes(p.lessonId),
  );
}
...
export function getSubjectsWithProgress(userId: string): SubjectWithProgress[] {
  return subjects.map((subject) => {
    ...
    const userProgress = lessonProgress.filter(
      (p) =>
        p.userId === userId && subjectLessons.some((l) => l.id === p.lessonId),
    );
...
export function getInProgressCourses(userId: string): SubjectWithProgress[] {
  return getSubjectsWithProgress(userId).filter(
