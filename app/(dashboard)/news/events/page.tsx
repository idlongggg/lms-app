...
const typeConfig: Record<string, { label: string; color: string; bgColor: string; icon: typeof Trophy }> = {
  competition: {
    label: "Thi đấu",
    color: "text-red-700",
    bgColor: "bg-red-100",
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <select className="border-2 border-border bg-background px-4 py-2 font-medium">
      ...
      {/* Featured Event */}
      {upcomingEvents[0] && (
        <div className="border-2 border-primary bg-primary/10 p-6 shadow-sm">
          <div className="flex flex-col gap-6 lg:flex-row">
            <div className="flex h-32 w-32 shrink-0 items-center justify-center border-2 border-border bg-primary text-6xl">
            ...
            <div className="flex-1">
              ...
              <p className="mt-2 text-muted-foreground">{upcomingEvents[0].description}</p>
            ...
            <div className="flex shrink-0 flex-col gap-2">
              <button className="border-2 border-border bg-primary px-6 py-3 font-medium shadow-xs transition-all hover:-translate-x-0.5 hover:-translate-y-0.5 hover:shadow-sm">
                Đăng ký ngay
              </button>
              <button className="border border-border bg-background px-6 py-2 text-sm">
                Thêm vào lịch
              </button>
            </div>
          </div>
        ...
        <div className="grid gap-4 md:grid-cols-2">
          {upcomingEvents.slice(1).map((event) => {
            ...
            return (
              <div
                key={event.id}
                className="border-2 border-border bg-background shadow-sm transition-all hover:-translate-x-0.5 hover:-translate-y-0.5 hover:shadow-md"
              >
                <div className="flex gap-4 p-4">
                  <div className="flex h-16 w-16 shrink-0 items-center justify-center border-2 border-border bg-muted text-3xl">
                    {event.image}
                  </div>
                  <div className="flex-1">
                    <div className="flex flex-wrap items-center gap-2">
        ...
        <div className="space-y-2">
          {pastEvents.map((event) => {
            ...
            return (
              <div
                key={event.id}
                className="flex items-center gap-4 border-2 border-border bg-background p-3 opacity-70"
              >
                <div className="flex h-10 w-10 shrink-0 items-center justify-center border border-border bg-muted text-xl">
                  {event.image}
                </div>
                <div className="flex-1">
