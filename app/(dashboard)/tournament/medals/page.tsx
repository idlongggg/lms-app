    ...
    <div className="space-y-8">
      ...
      <div className="grid gap-4 sm:grid-cols-5">
        {["Bronze", "Silver", "Gold", "Platinum", "Diamond"].map((rarity) => {
          const count = medals.filter(
            (m) => m.rarity === rarity && m.earned
          ).length;
          const total = medals.filter((m) => m.rarity === rarity).length;
          return (
            <div
              key={rarity}
              className="flex items-center gap-3 border-2 border-border bg-background p-3 shadow-sm"
            >
              <div
                className={`flex h-10 w-10 items-center justify-center border-2 border-border ${rarityColors[rarity]}`}
              >
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {medals.map((medal) => (
          <div
            key={medal.id}
            className={`relative border-2 bg-background p-4 shadow-sm transition-all ${
              medal.earned
          ...
          >
            {/* Rarity Badge */}
            <span
              className={`absolute right-2 top-2 px-2 py-0.5 text-xs font-medium text-white ${rarityColors[medal.rarity]}`}
            >
              {medal.rarity}
            </span>
            ...
            <div
              className={`mx-auto flex h-16 w-16 items-center justify-center border-2 text-3xl ${
                medal.earned
                  ? `${rarityBorders[medal.rarity]} bg-muted`
                  : "border-border bg-muted"
              }`}
            >
              {medal.earned ? medal.icon : <Lock className="h-8 w-8 text-muted-foreground" />}
            </div>
            ...
            {medal.earned ? (
              <p className="mt-3 text-center text-xs text-muted-foreground">
                Đạt được:{" "}
                {new Date(medal.earnedDate!).toLocaleDateString("vi-VN")}
              </p>
            ) : (
              <div className="mt-3">
