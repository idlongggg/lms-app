"use client";

import { BarChart } from "@/components/retroui/charts/BarChart";
import { Trophy, Medal, Crown, Award, Star } from "lucide-react";

const leaderboardData = [
  { rank: 1, name: "ProCoder99", score: 12500, wins: 85, avatar: "👨‍💻" },
  { rank: 2, name: "ReactMaster", score: 11200, wins: 78, avatar: "⚛️" },
  { rank: 3, name: "JSNinja", score: 10800, wins: 72, avatar: "🥷" },
  { rank: 4, name: "TypeHero", score: 9500, wins: 65, avatar: "📘" },
  { rank: 5, name: "CodeWarrior", score: 9200, wins: 63, avatar: "⚔️" },
  { rank: 6, name: "DevQueen", score: 8900, wins: 60, avatar: "👑" },
  { rank: 7, name: "ByteKing", score: 8600, wins: 58, avatar: "🤴" },
  { rank: 8, name: "AlgoAce", score: 8300, wins: 55, avatar: "🎯" },
  { rank: 9, name: "WebWizard", score: 8000, wins: 52, avatar: "🧙" },
  { rank: 10, name: "StackStar", score: 7800, wins: 50, avatar: "⭐" },
];

const weeklyTopData = [
  { name: "ProCoder99", points: 450 },
  { name: "ReactMaster", points: 380 },
  { name: "JSNinja", points: 350 },
  { name: "TypeHero", points: 320 },
  { name: "CodeWarrior", points: 280 },
];

const myStats = {
  rank: 42,
  score: 3200,
  wins: 28,
  percentile: "Top 15%",
};

export default function LeaderboardPage() {
  return (
    <div className="space-y-8">
      {/* Header */}
      <div>
        <h1 className="text-3xl font-bold">Bảng xếp hạng</h1>
        <p className="text-muted-foreground">
          Xem thứ hạng của bạn và các đối thủ hàng đầu
        </p>
      </div>

      {/* My Rank Card */}
      <div className="border-2 border-primary bg-primary/10 p-6 shadow-sm">
        <div className="flex flex-col items-center gap-6 sm:flex-row">
          <div className="flex h-20 w-20 items-center justify-center border-2 border-border bg-primary text-4xl font-bold">
            #{myStats.rank}
          </div>
          <div className="flex-1 text-center sm:text-left">
            <h2 className="text-xl font-bold">Thứ hạng của bạn</h2>
            <div className="mt-2 flex flex-wrap justify-center gap-4 sm:justify-start">
              <div className="text-center">
                <p className="text-2xl font-bold">{myStats.score}</p>
                <p className="text-sm text-muted-foreground">Điểm</p>
              </div>
              <div className="text-center">
                <p className="text-2xl font-bold">{myStats.wins}</p>
                <p className="text-sm text-muted-foreground">Thắng</p>
              </div>
              <div className="text-center">
                <p className="text-2xl font-bold">{myStats.percentile}</p>
                <p className="text-sm text-muted-foreground">Percentile</p>
              </div>
            </div>
          </div>
          <button className="border-2 border-border bg-background px-6 py-3 font-medium shadow-xs transition-all hover:-translate-x-0.5 hover:-translate-y-0.5 hover:shadow-sm">
            Thách đấu ngay
          </button>
        </div>
      </div>

      <div className="grid gap-6 lg:grid-cols-3">
        {/* Leaderboard Table */}
        <div className="border-2 border-border bg-background shadow-sm lg:col-span-2">
          <div className="border-b-2 border-border bg-muted px-6 py-4">
            <div className="flex items-center justify-between">
              <h2 className="font-bold">Top 10 toàn thời gian</h2>
              <select className="border border-border bg-background px-2 py-1 text-sm">
                <option>Toàn thời gian</option>
                <option>Tháng này</option>
                <option>Tuần này</option>
              </select>
            </div>
          </div>
          <div className="divide-y-2 divide-border">
            {leaderboardData.map((player) => (
              <div
                key={player.rank}
                className={`flex items-center gap-4 p-4 transition-colors hover:bg-muted/50 ${
                  player.rank <= 3 ? "bg-primary/5" : ""
                }`}
              >
                {/* Rank */}
                <div className="flex h-10 w-10 shrink-0 items-center justify-center border-2 border-border font-bold">
                  {player.rank === 1 ? (
                    <Crown className="h-5 w-5 text-yellow-500" />
                  ) : player.rank === 2 ? (
                    <Medal className="h-5 w-5 text-gray-400" />
                  ) : player.rank === 3 ? (
                    <Medal className="h-5 w-5 text-amber-600" />
                  ) : (
                    player.rank
                  )}
                </div>

                {/* Avatar */}
                <div className="flex h-10 w-10 items-center justify-center border-2 border-border bg-muted text-xl">
                  {player.avatar}
                </div>

                {/* Name */}
                <div className="flex-1">
                  <p className="font-medium">{player.name}</p>
                  <p className="text-sm text-muted-foreground">
                    {player.wins} thắng
                  </p>
                </div>

                {/* Score */}
                <div className="text-right">
                  <p className="font-bold">{player.score.toLocaleString()}</p>
                  <p className="text-sm text-muted-foreground">điểm</p>
                </div>
              </div>
            ))}
          </div>
          <div className="border-t-2 border-border p-4">
            <button className="w-full text-center text-sm font-medium text-muted-foreground hover:text-foreground">
              Xem thêm
            </button>
          </div>
        </div>

        {/* Weekly Top Chart */}
        <div className="border-2 border-border bg-background shadow-sm">
          <div className="border-b-2 border-border bg-muted px-6 py-4">
            <h2 className="font-bold">Top tuần này</h2>
          </div>
          <div className="p-4">
            <BarChart
              data={weeklyTopData}
              index="name"
              categories={["points"]}
              alignment="horizontal"
              fillColors={["var(--primary)"]}
              className="h-64"
              valueFormatter={(value) => `${value} pts`}
            />
          </div>
        </div>
      </div>

      {/* Rewards Info */}
      <div className="border-2 border-border bg-muted/50 p-6">
        <h3 className="mb-4 flex items-center gap-2 font-bold">
          <Award className="h-5 w-5" />
          Phần thưởng xếp hạng
        </h3>
        <div className="grid gap-4 sm:grid-cols-3">
          <div className="flex items-center gap-3 border-2 border-border bg-background p-3">
            <Crown className="h-8 w-8 text-yellow-500" />
            <div>
              <p className="font-medium">Top 1</p>
              <p className="text-sm text-muted-foreground">5,000 xu/tháng</p>
            </div>
          </div>
          <div className="flex items-center gap-3 border-2 border-border bg-background p-3">
            <Medal className="h-8 w-8 text-gray-400" />
            <div>
              <p className="font-medium">Top 2-3</p>
              <p className="text-sm text-muted-foreground">2,500 xu/tháng</p>
            </div>
          </div>
          <div className="flex items-center gap-3 border-2 border-border bg-background p-3">
            <Star className="h-8 w-8 text-amber-500" />
            <div>
              <p className="font-medium">Top 10</p>
              <p className="text-sm text-muted-foreground">1,000 xu/tháng</p>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
}
