        ...
        {classes.map((cls) => {
          ...
          return (
            ...
            >
              ...
              <div className="border-border border-b-2 p-4">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    <div className="border-border bg-primary flex h-14 w-14 items-center justify-center border-2 text-xl font-bold">
                      {cls.grade}
                    </div>
                    <div>
                  ...
                  <div className="flex items-center gap-2">
                    <span
                      className={`border-border border px-3 py-1 text-sm font-medium ${
                        cls.status === "ACTIVE"
              ...
              {/* Stats Row */}
              <div className="border-border grid grid-cols-4 border-b-2">
                <div className="border-border border-r-2 p-4 text-center">
                  <p className="text-2xl font-bold">{cls.studentCount}</p>
                  <p className="text-muted-foreground text-xs">Học sinh</p>
                </div>
                <div className="border-border border-r-2 p-4 text-center">
                  <p className="text-2xl font-bold text-green-500">
                    {progress?.averageProgress || 0}%
                  </p>
                  <p className="text-muted-foreground text-xs">Tiến độ TB</p>
                </div>
                <div className="border-border border-r-2 p-4 text-center">
              ...
              <div className="grid gap-4 p-4 lg:grid-cols-2">
                ...
                <div>
                  ...
                  {progress?.topPerformers &&
                  progress.topPerformers.length > 0 ? (
                      ...
                      {progress.topPerformers
                        ...
                        .map((student, index) => (
                          <div
                            ...
                            className="flex items-center gap-3 rounded bg-green-50 p-2 dark:bg-green-950"
                          >
                  ...
                  progress.needsAttention.length > 0 ? (
                    <div className="space-y-2">
                      {progress.needsAttention.slice(0, 3).map((student) => (
                        <div
                          key={student.studentId}
                          className="flex items-center gap-3 rounded bg-red-50 p-2 dark:bg-red-950"
                        >
                          ...
                          <div className="text-muted-foreground text-xs">
                            {Math.round(
                              (new Date().getTime() -
                                new Date(student.lastActive).getTime()) /
                                (1000 * 60 * 60 * 24),
                            )}{" "}
                            ngày trước
                          </div>
                    ...
                    <div className="rounded bg-green-50 p-4 text-center dark:bg-green-950">
                      <p className="text-sm text-green-600">
                        ✓ Tất cả học sinh đang tiến bộ tốt!
                      </p>
                    </div>
                  )}
                </div>
              ...
              {/* Actions */}
              <div className="border-border flex gap-2 border-t-2 p-4">
                <Link
                  href={`/learning/class-progress?classId=${cls.id}`}
                  className="border-border bg-primary flex-1 border-2 px-4 py-2 text-center font-medium shadow-sm transition-all hover:-translate-x-0.5 hover:-translate-y-0.5 hover:shadow-md"
                >
                  <Eye className="mr-2 inline h-4 w-4" />
                  Xem chi tiết tiến độ
                </Link>
                <button className="border-border bg-muted hover:bg-muted/80 border-2 px-4 py-2 font-medium transition-all">
