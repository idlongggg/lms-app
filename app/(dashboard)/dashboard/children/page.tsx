  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        {CHILDREN.map((child) => (
          ...
          >
            <Card.Content className="p-6">
              <div className="mb-6 flex items-start justify-between">
                <div className="flex items-center gap-4">
                  <Avatar className={`h-14 w-14 border-none shadow-sm`}>
                    <Avatar.Fallback
                      className={`${child.avatarColor} text-xl font-bold text-white`}
                    >
                      {child.name.charAt(0)}
                    </Avatar.Fallback>
                  </Avatar>
              ...
              <div className="border-border mb-4 grid grid-cols-3 gap-2 border-t border-b py-4">
                <div className="text-center">
                  <div className="text-lg font-bold">
                    {child.stats.avgScore}%
                  </div>
                  <div className="text-muted-foreground flex items-center justify-center gap-1 text-xs">
                    <Award className="h-3 w-3" /> Score
                  </div>
                </div>
                <div className="border-border border-r border-l text-center">
                  <div className="text-lg font-bold">
                    {child.stats.completedLessons}
                  </div>
                  <div className="text-muted-foreground flex items-center justify-center gap-1 text-xs">
        ...
        <button className="border-border text-muted-foreground hover:text-primary hover:border-primary hover:bg-primary/5 flex min-h-[280px] flex-col items-center justify-center rounded-xl border-2 border-dashed p-6 transition-all">
          <div className="bg-secondary mb-4 flex h-12 w-12 items-center justify-center rounded-full transition-transform group-hover:scale-110">
