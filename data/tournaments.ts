...
export const tournaments: Tournament[] = [
  {
    ...
    rules:
      "Trả lời 10 câu hỏi trong 5 phút. Mỗi câu đúng +10 điểm, sai -2 điểm.",
  ...
  },
  {
    ...
    bannerUrl:
      "https://placehold.co/800x400/2ecc71/white?text=English+Challenge",
    maxParticipants: 50,
    entryFee: 100,
    minLevel: 5,
    status: "REGISTRATION",
...
export function getUserTournamentHistory(userId: string): MatchHistoryEntry[] {
  const userParticipations = participants.filter(
    (p) => p.userId === userId && p.score !== null,
  );
  ...
  return userParticipations.map((p) => {
    ...
    const medal = userMedal
      ? medals.find((m) => m.id === userMedal.medalId)
      : null;
    ...
    return {
      ...
      result:
        (p.rank || 0) <= 3 ? "win" : (p.rank || 0) <= 10 ? "draw" : "loss",
...
export function getTournamentCards(): TournamentCard[] {
  return tournaments.map((t) => {
    ...
    return {
      ...
      banner:
        t.bannerUrl || "https://placehold.co/800x400/333/white?text=Tournament",
      status: t.status,
